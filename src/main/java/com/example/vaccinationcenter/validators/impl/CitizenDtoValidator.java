  ...
  private static class PostRequestValidator implements RequestDtoValidator<CitizenDto> {
    ...
    public boolean validate(CitizenDto citizen, ConstraintValidatorContext context) {
      ...
      if (StringUtils.isBlank(citizen.getCity())) {
        flag = false;
        context.disableDefaultConstraintViolation();
        context.buildConstraintViolationWithTemplate("Citizen city required").
      ...
      if (!(citizen.getCity().length() > 2 && citizen.getName().length() < 30)) {
        ...
        context.disableDefaultConstraintViolation();
        context.
            buildConstraintViolationWithTemplate("Citizen city should between 2 and 30 in length")
            .addPropertyNode("city")
            .addConstraintViolation();
      }
